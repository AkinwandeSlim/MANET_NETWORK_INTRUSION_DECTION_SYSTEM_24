...
def getConfigData():
    ...
    fileIndicies=list(map(int,input("Enter Option Numbers for files to track with comma (,):").split(",")))

    for index in fileIndicies:
        data["files_to_track"].append(files_in_directory[index])

    print("\n")
    displayMessage("Configuration Completed Successfully")
    print("\n")
